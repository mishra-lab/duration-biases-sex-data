...
%---------------------------------------------------------------------------------------------------
\paragraph{Sampling}
% LW: can u clarify if u start of ur adjustment from base estimate (raw median=> raw mean)
%     or if u start of ur adjustment from the sampling-adjusted mean (from the reference 10)?
%     looks like the latter? if so - i would make it clear -
%     the RDS sampling adjustment was done at the individual -data level by study 10.
%     yet- i think the limitation was data reported as categorical (% in each category)
%     => based on which u aimed to come up with a distribution - if i understand correctly
% JK: yes that's correct, we basically start over using the RDS-adjusted proportions,
%     and try to fit a model to those proportions; I've edited to try and make this more clear.
...
%---------------------------------------------------------------------------------------------------
\paragraph{Censoring}
% LW: assumption on uniform censoring. maybe make the asumptions explicit somewhere
% JK: nice! I've added below.
...
%---------------------------------------------------------------------------------------------------
\paragraph{Summary}
% LW: Consider moving the primary paramter of interest at the begining of the methods section.
%     like u had in ur Figure 2 footnote.
%     walk through what is the red circle (target estimate of interest) etc.
% JK: in an earlier draft, I had something similar to this,
%     but it read awkward because we have not introduced the bias mechanisms,
%     so the reader may not know why \bar{D} is different from \bar{d}, etc.
